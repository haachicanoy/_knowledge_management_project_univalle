...
# Transferencia del conocimiento  -
#
#  Adquisicion Conocimiento Interno & Externo ----> Gestion conocimiento
#                                      
# Uso del Conocimiento            -
#
# ------------------------------------------------------- #

km_df <- km_data %>% dplyr::select(P9_1:P9_6,     # Gestion conocimiento
                                   P7_1:P7_5,     # Transferencia del conocimiento 
                                   P5_1:P5_6,     # Adquisicion Conocimiento Externo
                                   P6_1:P6_5,     # Adquisicion Conocimiento Interno
                                   P8_1:P8_5)     # Uso del conocimiento
# rows of the path matrix
A.Conocimiento = c(0,0,0,0)
T.conocimiento = c(0,0,0,0)
G.Conocimiento = c(1,1,0,0)
U.conocimiento = c(0,0,1,0)


# path matrix (inner model)
foot_path = rbind(A.Conocimiento, T.conocimiento, G.Conocimiento, U.conocimiento)

# add column names
colnames(foot_path) = rownames(foot_path)

# blocks of indicators (outer model)
foot_blocks = list(1:11, 12:16, 17:22, 23:ncol(km_df))
